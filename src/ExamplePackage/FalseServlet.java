...
public class FalseServlet extends HttpServlet {
	...
	if(category.equals("salarycheck")){
		try {
			conn.CreateSalaryCheck(tid, aid, false, uid);
		...
		try {
			conn.CreateOtherIncomeCheck(tid, aid, false, uid);
          ...
          conn.CreateBorrowCheck(tid, aid, false, uid);
		...
		try{
        conn.CreateOtherExpenditureCheck(tid, aid, false, uid);
